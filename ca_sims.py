...
for i, data in enumerate(datas):
    ...
    for k in range(num_networks):
        ...
        res = nd_p.gmm_gillesp(samples,partitions=partitions,taus=taus, iterations=48, num_infec=5)
        with open(f'duration+ages/seir_sims/{data}_{k}_nodur.json','w') as f:
            json.dump(res, f)
        
        res = nd_p.sbm_gillesp(contact_matrix=cm, partitions=partitions, taus=taus, iterations=48, num_infec=5)
        with open(f'duration+ages/seir_sims/{data}_{k}_sbm.json','w') as f:
            json.dump(res, f)
