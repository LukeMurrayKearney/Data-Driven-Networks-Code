...
def simulate(network, partitions=None, beta=0.1, inv_gamma=7, prop_infec=1e-3, scaling="None"):
    ## Need to write some sensible code for this, depending on if we want to give params or a network
    if partitions == None:
        partitions = [0 for _ in range(max(network['ages']))]
        for group in network['ages']:
            partitions[group] += 1
    else:
        partitions = [int(a) for a in partitions]
    parameters = [beta, inv_gamma]
    return nd_r.small_sellke(len(network['ages']), network['adjacency_matrix'], network['ages'], parameters, prop_infec, scaling)
