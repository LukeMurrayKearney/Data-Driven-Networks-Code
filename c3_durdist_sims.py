...
# taus = np.arange(.01,.21,.01)
taus = np.arange(.51,1.01,.025)
...
for i, data in enumerate(datas):
    ...
    for k in range(num_networks):
        ...
        res = nd_p.gmm_dur_gillesp(samples,partitions=partitions,num_dur=3, taus=taus, iterations=48,props=props.tolist(), num_infec=5)
        with open(f'duration+ages/sims/{data}_{k+40}_d1.json','w') as f:
            json.dump(res, f)
