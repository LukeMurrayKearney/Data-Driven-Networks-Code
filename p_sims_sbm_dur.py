...
taus1 = np.arange(0.059,0.095,0.005)
taus2 = np.arange(0.132,0.172,0.005)
taus3 = np.arange(0.2125,0.2925,0.01)
taus = np.concatenate((taus1, taus2, taus3))
# taus = np.array([.045,.05,.055,.06,.065,.07,.075,.08,.085,.13,.14,.15,.16,.17,.23,.24,.25,.26,.27,.28,.29,.3])+.1
...
for i, data in enumerate(datas):
    ...
    for k in range(num_networks):
        ...
        with open(f'duration+ages/seir_sims/{data}_{k+160}_{model}_fin.json','w') as f:
            json.dump(res, f)
