...
taus2 = np.arange(.00001, .0002, .00001)
...
for i, data in enumerate(datas):
    ...
    for k in range(num_networks):
        ...
        with open(f'duration+ages/seir_sims/{data}_{k+200}_nodur_gen1.json','w') as f:
            json.dump(res, f)
        res = nd_p.gmm_gillesp(samples,partitions=partitions,taus=taus2, iterations=48, num_infec=1)
        with open(f'duration+ages/seir_sims/{data}_{k+220}_nodur_gen1.json','w') as f:
            json.dump(res, f)
