...
taus1 = np.arange(.005, .075, 0.005)
taus3 = np.arange(0.705, .1, 0.01)
taus2 = np.arange(.1, 2.5, .1)
...
for i, data in enumerate(datas):
    ...
    for k in range(num_networks):
        ...
        with open(f'duration+ages/seir_sims/{data}_{k+20}_noage_fin.json','w') as f:
            json.dump(res, f)